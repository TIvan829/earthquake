...
async def help(ctx):
	embed = discord.Embed(
		title = "地震Bot使用方法",
		description = f"{PREFIX}help | 傳送這則沒意義的訊息\n{PREFIX}setq | 創建名為`🛑地震速報`的頻道(會傳送地震訊息)",
...
async def on_ready(): #當啟動時
	...
	#開啟時測試地震訊息
	if READYMSG == 'true': #如果設置為開啟
		for guild in bot.guilds:
			for channel in guild.channels:
				if channel.name == "🛑地震速報": #偵測地震資訊
	...
	while True: #重複執行 偵測'60秒前'的資訊,做比對
		data = requests.get(URL) #開啟API
		data_json = data.json()
		eq = data_json['records']['earthquake'] #轉換成json格式
		for i in eq:
		...
		for i in eq:
			content = i['reportContent'] #content=地震內容
			picture=i['reportImageURI'] #picture=地震資訊圖片
		if str(lcontent) != str(content): #如果60秒前和60秒後的內容不同
